...
function parseCommanders(data)
{
	...
	for (var i = 0; i < inputs.length; i++)
	{
		var id = inputs[i].replace(/x\d+/ig, "").trim();
		var nbr = +inputs[i].match(/x\d+/ig)[0].replace(/x/g, "");
...
function validateCommanders(input, username)
{
	...
	if (/[^x\d\s\,]/.test(input) === true || /\d/.test(input) === false)
	{
		rw.log("Commanders' input was incorrect: " + input + ". ");
		result.data = "The input must be formated like so, any number of times: `<ID> x<Amount>`. For example, `5 x10, 54 x2, 87 x1`.";
		return result;
	}
	...
	{
		rw.log("Commanders' input resulted in incorrect units: " + input + ".");
		result.data = "The input must contain both an ID and a number, as well as a comma between each set, like so: `5 x10, 54 x2, 87 x1`.";
		return result;
	}
...
function validateUnits(input, username)
{
	...
	{
		rw.log("Units' input was incorrect: " + input + ". ");
		result.data = "The input must be formated like so, any number of times: `<ID> x<Amount>`. For example, `1 x50, 18 x50`.";
		return result;
	}
	...
	{
		rw.log("Units' input resulted in incorrect units: " + input + ".");
		result.data = "The input must contain both an ID and a number, as well as a comma between each set, like so: `1 x50, 18 x50`.";
		return result;
	}
